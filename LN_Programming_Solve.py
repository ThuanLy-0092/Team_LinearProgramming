...
class Problem:
    ...
    def display(self):
        ...
        # Sử dụng bảng để căn chỉnh các ràng buộc
...
class Dict:
    ...
    def display(self):
        ...
        # Sử dụng HTML để căn chỉnh bảng và giảm khoảng cách
        # <hr>  style='margin:10px 0;' điều chỉnh khoảng cách phần hàm mục tiêu và ràng buộc
        ...
        for i, row in enumerate(self.eq_constrs_matrix):
            ...
            constraint_str = f"<pre>{constraint_str}</pre>"  # Thêm thẻ <pre> để giữ nguyên định dạng văn bản
...
class Phase2(Dict):
    ...
    def solve(self):
        ...
        while True:
            ...
            if all_positive:
                # Kiểm tra xem có biến cơ sở nào có hệ số c_j = 0 và không thuộc biến ở dạng "x_j+" hoặc "x_j-"
                zero_cj = [i for i in range(1, len(self.indep_var_obj)) if self.indep_var_obj[i] == 0]
                for col in zero_cj:
                    for i in range(1, len(self.dep_var)):
                        if self.dep_var[i].endswith('+') or self.dep_var[i].endswith('-'):
                            # Nếu có, đặt status là 1 và kết thúc
                            self.set_status(1)
                            return
                # Nếu không có biến nào thỏa mãn điều kiện trên, đặt status là 2 và kết thúc
                self.set_status(2)
                return
...
class S2P_Method:
    ...
    def display(self):
        if self.get_status() == 1:
            ...
            # Hiển thị giải pháp tối ưu dưới dạng bảng

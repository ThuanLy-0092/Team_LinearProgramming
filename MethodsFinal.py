...
class Phase1(Dict):
    ...
    def solve(self):
        ...
        while True:
            ...
            if all_positive:
                self.set_status(1 if self.indep_var_obj[0] == 0 else 2) # If c_0 = 0, the problem is feasible, otherwise it's infeasible
                self.set_times(times)
                return

            # Get the first negative value of c_j, which means the pivot column
            ...
            # Decending variable priority order: x0 -> x1+ -> x1- -> ... xn- -> w1 -> w2 -> ... wn
            for i in range(len(self.eq_constrs_matrix)):
                if self.eq_constrs_matrix[i][col] >= 0:
                    continue
                if self.eq_constrs_matrix[i][0] / -self.eq_constrs_matrix[i][col] == min_val:
